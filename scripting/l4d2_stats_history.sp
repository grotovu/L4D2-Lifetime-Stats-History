...
ConVar g_hCvarNames[MAX_BOT_CHARS];
...
//					PLUGIN START & END
// ====================================================================================================
public Plugin myinfo = { name = "[L4D2] Lifetime Stats History", author = "EagleRaviOrange", version = PLUGIN_VERSION }
...
public void OnPluginStart()
{
	...
	g_cvEnable = CreateConVar("l4d2_stats_enable", "1", "Enable the Stats History tracking plugin?");
	g_cvPrintMode = CreateConVar("l4d2_stats_print_mode", "2", "0=Disabled, 1=Print at Finale Win, 2=Print at end of every chapter.");
	g_cvSaveOnDisconnect = CreateConVar("l4d2_stats_save_disconnect", "1", "Should stats be saved to the database when a player disconnects? (Set to 0 during testing/modding)", _, true, 0.0, true, 1.0);
	g_cvPrintDamageReceived = CreateConVar("l4d2_stats_print_damage_received", "1", "Should damage received statistics be printed in show commands and log sheets? (0=No, 1=Yes)");
	g_hCvarNames[0] = CreateConVar("l4d2_stats_name_nick", "", "Custom name for Nick", FCVAR_NOTIFY);
	g_hCvarNames[1] = CreateConVar("l4d2_stats_name_rochelle", "", "Custom name for Rochelle", FCVAR_NOTIFY);
	g_hCvarNames[2] = CreateConVar("l4d2_stats_name_coach", "", "Custom name for Coach", FCVAR_NOTIFY);
	g_hCvarNames[3] = CreateConVar("l4d2_stats_name_ellis", "", "Custom name for Ellis", FCVAR_NOTIFY);
	g_hCvarNames[4] = CreateConVar("l4d2_stats_name_bill", "", "Custom name for Bill", FCVAR_NOTIFY);
	g_hCvarNames[5] = CreateConVar("l4d2_stats_name_zoey", "", "Custom name for Zoey", FCVAR_NOTIFY);
	g_hCvarNames[6] = CreateConVar("l4d2_stats_name_francis", "", "Custom name for Francis", FCVAR_NOTIFY);
	g_hCvarNames[7] = CreateConVar("l4d2_stats_name_louis", "", "Custom name for Louis", FCVAR_NOTIFY);
...
void InitDatabase()
{
    ...
    g_hDatabase.Query(SQL_Callback_TableCreated, 
        ...
        ... "jockey_deadstops INTEGER DEFAULT 0, hunter_deadstops INTEGER DEFAULT 0,"
		 ... "witches_startled INTEGER DEFAULT 0, times_boomed INTEGER DEFAULT 0"
        ... ");"
    );
...
void AddPlayerToTransaction(int client, Transaction hTr)
{
    ...
    g_hDatabase.Format(sQuery, sizeof(sQuery), 
        ...
        "spitter_killed_pre_spat, jockey_deadstops, hunter_deadstops, " ...
		"witches_startled, times_boomed " ...
        ") VALUES ('%s', %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d);",
        auth, 
		...
		g_Lifetime[client].witchesStartled, g_Lifetime[client].timesBoomed
    );
    hTr.AddQuery(sQuery);
...
void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast) {
    ...
    int zombieClass = GetEntProp(victim, Prop_Send, "m_zombieClass");
    switch (zombieClass) {
        case 1: { ADD_STAT(attacker, killsSmoker); UpdateWeaponStat(attacker, clean, 5); }
        case 2: { ADD_STAT(attacker, killsBoomer); UpdateWeaponStat(attacker, clean, 6); }
        case 3:
        {
            ADD_STAT(attacker, killsHunter);
            UpdateWeaponStat(attacker, clean, 7);
            
            if (g_iPendingSkeetAttacker[victim] == attacker) {
                ADD_STAT(attacker, hunterSkeets);
                UpdateWeaponStat(attacker, clean, 15);
            }
        }
        case 4: { ADD_STAT(attacker, killsSpitter); UpdateWeaponStat(attacker, clean, 8); }
        case 5: { ADD_STAT(attacker, killsJockey); UpdateWeaponStat(attacker, clean, 9); }
        case 6:
        {
        ...
        case 8: { ADD_STAT(attacker, killsTank); UpdateWeaponStat(attacker, clean, 11); }
    }
...
void Event_SpitterKilled(Event event, const char[] name, bool dontBroadcast) {
    ...
    bool hasSpat = (victim > 0 && victim <= MaxClients) ? g_bSpitterSpat[victim] : event.GetBool("has_spit");
    
    if (IsSurvivor(attacker) && !hasSpat) {
        ADD_STAT(attacker, spitterKilledPreSpat);
        
        int weaponID = g_iClientActiveWeaponID[attacker];
        if (weaponID != -1) {
            UpdateWeaponStatID(attacker, weaponID, 20);
        }
    }
}
...
void GetBotPrettyName(int character, char[] buffer, int maxlen) {
    ...
    if (character < 8) {
        g_hCvarNames[character].GetString(buffer, maxlen);
        if (buffer[0] != '\0') return; 
    }

    for (int i = 1; i <= MaxClients; i++) {
        if (IsClientInGame(i) && IsFakeClient(i) && GetSurvivorCharacterInternal(i) == character) {
...
public Action CmdShowHistory(int client, int args)
{
    ...
    PrintToConsole(client, " Times Boomed On:    %d\n", g_Lifetime[client].timesBoomed);
...
void GeneratePrintFile(int client, bool isAuto)
{
    ...
    hFile.WriteLine(lineBuffer);
    Format(lineBuffer, sizeof(lineBuffer), "Times Boomed On:    %d\n", g_Lifetime[client].timesBoomed);
    hFile.WriteLine(lineBuffer);
...
void GenerateCampaignPrintFile(int client)
{
    ...
    for (int i = 0; i < MAX_BOT_CHARS; i++) {
        if (g_BotCampaign[i].totalSeconds > 0) {
            ...
            if (g_BotCampaign[i].hunterSkeets > 0 || g_BotCampaign[i].witchCrowns > 0 || g_BotCampaign[i].tongueCuts > 0 || g_BotCampaign[i].selfRescues > 0 || g_BotCampaign[i].chargerLevels > 0 || g_BotCampaign[i].rockSkeets > 0 || g_BotCampaign[i].spitterKilledPreSpat > 0 || g_BotCampaign[i].witchesStartled > 0 || g_BotCampaign[i].timesBoomed > 0 || g_BotCampaign[i].jockeyDeadstops > 0 || g_BotCampaign[i].hunterDeadstops > 0) {
                ...
                hFile.WriteLine(lineBuffer);
                Format(lineBuffer, sizeof(lineBuffer), "                   Jockey Deadstops: %-3d | Hunter Deadstops: %-3d", g_BotCampaign[i].jockeyDeadstops, g_BotCampaign[i].hunterDeadstops);
                hFile.WriteLine(lineBuffer);
            }
        ...
        if (g_BotCampaign[i].totalSeconds > 0) {
            ...
            if (g_BotCampaign[i].hunterSkeets > 0 || g_BotCampaign[i].witchCrowns > 0 || g_BotCampaign[i].tongueCuts > 0 || g_BotCampaign[i].selfRescues > 0 || g_BotCampaign[i].chargerLevels > 0 || g_BotCampaign[i].rockSkeets > 0 || g_BotCampaign[i].spitterKilledPreSpat > 0 || g_BotCampaign[i].witchesStartled > 0 || g_BotCampaign[i].timesBoomed > 0 || g_BotCampaign[i].jockeyDeadstops > 0 || g_BotCampaign[i].hunterDeadstops > 0) {
                PrintToConsole(client, "  Special Feats:   Skeets:%-3d | Crowns:%-3d | Cuts:%-3d | Self-Rescues:%-3d | Levels:%-3d | Rocks:%-3d | Pre-Spat:%-3d | Startled:%-3d | Boomed:%-3d",
                    g_BotCampaign[i].hunterSkeets, g_BotCampaign[i].witchCrowns, g_BotCampaign[i].tongueCuts, g_BotCampaign[i].selfRescues, g_BotCampaign[i].chargerLevels, g_BotCampaign[i].rockSkeets, g_BotCampaign[i].spitterKilledPreSpat, g_BotCampaign[i].witchesStartled, g_BotCampaign[i].timesBoomed);
                PrintToConsole(client, "                   Jockey Deadstops: %-3d | Hunter Deadstops: %-3d", g_BotCampaign[i].jockeyDeadstops, g_BotCampaign[i].hunterDeadstops);
            }
